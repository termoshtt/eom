    ...
    where F: ExplicitBuf + ModelSpec
{
        ...
        where S: DataMut<Elem = Self::Scalar>
    {
        ...
        let mut fx = self.f.rhs(x, &mut buf.ex);
        Zip::from(&mut *fx)
            .and(&buf.x)
